...
## 初始化流程
 ...
 4. 若 `config/private/secret` 有 `base` 欄位，依字典序執行 `config/<base>/db/*.sql`
...
`config/private/secret.ls` 的 `base` 欄位決定哪個 frontend/backend 目錄被載入，init 腳本也讀取這個欄位，自動執行 `config/<base>/db/` 下的自定義 SQL。多個 `.sql` 檔依字典序執行。
