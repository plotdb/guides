...
| id | valdef | 說明 |
|----|--------|------|
| `list` | generic | 陣列長度驗證（count-max/min/range） |
| `file` | generic | 檔案大小、數量、副檔名 |
| `image` | generic | 圖片尺寸、像素 |
| `string` | generic | 文字包含、排除、email、url、regex |
| `length` | generic | 字數長度（含多種計算方式） |
| `number` | generic | 數值比較 |
| `date` | generic | 日期、年齡 |
| `choice` | valdef/choice | 選項比較：is / is-not / is-any |
